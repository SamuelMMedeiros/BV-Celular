...
import { CartItem, Store, Product, ProductVariant } from "@/types";
...
const ProductDetails = () => {
    ...
    // Estado para Variação Selecionada
    ...
    useEffect(() => {
        ...
        setSelectedVariant(null); // Reseta seleção ao trocar de produto
    }, [productId, api]);
    ...
    // LÓGICA DE PREÇO (Base vs Variação)
    ...
    const isPromoValid =
        ...
        (!product.promotion_end_date ||
            !isBefore(new Date(product.promotion_end_date), new Date()));
    const discount =
        isPromoValid &&
    ...
    const isOutOfStock = currentQuantity !== undefined && currentQuantity <= 0;

    // Verifica se precisa selecionar variação
    const needsSelection = product.has_variations && !selectedVariant;
    ...
    const handleAddToCart = () => {
        if (needsSelection) {
            toast({
                ...
                description: "Escolha a cor/modelo para continuar.",
            });
            return;
        }
    ...
    const handleShare = async () => {
        const shareData = {
            title: product.name,
            text: `Confira ${product.name} na BV Celular!`,
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 xl:gap-16 mb-16">
                        ...
                        <div className="relative aspect-square w-full bg-muted/30 rounded-2xl overflow-hidden border">
                            ...
                            <div className="absolute top-4 left-4 flex flex-col gap-2 z-10">
                                ...
                                {!isOutOfStock &&
                                    isPromoValid &&
                                    discount > 0 && (
                                        <Badge className="bg-destructive text-destructive-foreground text-lg px-3 py-1 shadow-md w-fit">
                                            -{discount}%
                                        </Badge>
                                    )}
                                {!isOutOfStock &&
                                    ...
                                    product.promotion_end_date && (
                                        <Badge
                                            variant="secondary"
                                            className="bg-black/70 text-white backdrop-blur-md border-none shadow-md w-fit flex items-center gap-1"
                                        >
                            ...
                            <div className="absolute top-4 right-4 z-10">
                                <FavoriteButton
                                    ...
                                    className="bg-white/80 backdrop-blur-md shadow-md hover:bg-white"
                                />
                            </div>
                        </div>
                        {images.length > 1 && (
                    ...
                    <div className="flex flex-col space-y-6">
                        ...
                        <div className="p-6 bg-card border rounded-xl shadow-sm space-y-6">
                            ...
                            {/* SELEÇÃO DE VARIAÇÕES (CORES/MODELOS) */}
                                            ...
                                            {product.variants.map((variant) => {
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        ${
                                                            varStock <= 0
                                                                ? "opacity-50 cursor-not-allowed bg-muted text-muted-foreground decoration-slate-500 line-through"
                                                                : ""
                                                        }
                                                    `}
                                                    >
                            ...
                            {isOutOfStock && (
                                <div className="flex items-center justify-center gap-2 text-destructive text-sm font-medium mt-2">
