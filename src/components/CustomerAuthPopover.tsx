...
import { CardDescription } from '@/components/ui/card'; // CardDescription é usado aqui
import { useToast } from '@/hooks/use-toast'; // Para feedback

// Schema de validação
const formSchema = z.object({
  name: z.string().min(2, "Nome é obrigatório"),
  phone: z.string().regex(/^\d{10,11}$/, "Telefone inválido (10 ou 11 dígitos, apenas números)"),
});
type FormValues = z.infer<typeof formSchema>;

export const CustomerAuthPopover = () => {
  const { profile, isLoggedIn, login, logout, getGreeting } = useCustomerAuth();
  ...
  if (isLoggedIn) {
    return (
      <div className="flex items-center space-x-2 border-r pr-3">
        <span className="text-sm font-medium hidden sm:inline">{getGreeting()}</span>
    ...
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      ...
      <PopoverContent className="w-80 p-4">
        <div className="space-y-2">
          <h4 className="font-bold text-lg">Identificação do Cliente</h4>
          <CardDescription>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="mt-4 space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="customer-phone" className="flex items-center gap-1">
                <Phone className="h-4 w-4 text-muted-foreground" /> Telefone (DDD)
            </Label>
            <Input
              id="customer-phone"
              type="tel"
