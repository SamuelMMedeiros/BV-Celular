...
//
// === CÓDIGO COMPLETO PARA: src/components/ProductCard.tsx ===
//
...
export const ProductCard = ({ product }: ProductCardProps) => {
    ...
    // Apenas exibimos specs se for aparelho ou se tiver dados explicitos
    ...
    return (
        ...
        >
            ...
            <div className="relative aspect-square overflow-hidden bg-muted/50 shrink-0">
                ...
                {/* BADGES */}
                {isOutOfStock && (
                    <Badge className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-black text-white z-30 font-bold">
                        ESGOTADO
                    </Badge>
                )}

                {!isOutOfStock && isPromoValid && discount > 0 && (
                    <Badge className="absolute left-2 top-2 bg-destructive text-destructive-foreground z-10 font-bold">
                        -{discount}%
                    </Badge>
                )}

                {!isOutOfStock && isPromoValid && product.promotion_end_date && (
                    <Badge variant="secondary" className="absolute left-2 bottom-2 z-10 text-[10px] flex items-center gap-1 bg-black/70 text-white hover:bg-black/80 border-none backdrop-blur-sm">
                        <Clock className="h-3 w-3" />
                        Até {format(new Date(product.promotion_end_date), "dd/MM")}
                    </Badge>
                )}
            </div>
            ...
            <div className="p-4 flex flex-col flex-1 gap-2">
                ...
                <div className="flex items-center justify-between text-xs text-muted-foreground uppercase tracking-wide">
                    <span className="font-semibold">{product.brand || "Genérico"}</span>
                ...
                {hasSpecs ? (
                    <div className="flex flex-wrap gap-2 text-xs text-muted-foreground mt-1">
                        {product.storage && (
                            <Badge variant="outline" className="font-normal bg-transparent text-muted-foreground border-muted-foreground/30 px-1.5">
                                <HardDrive className="h-3 w-3 mr-1"/>{product.storage}
                            </Badge>
                        )}
                        {product.ram && (
                            <Badge variant="outline" className="font-normal bg-transparent text-muted-foreground border-muted-foreground/30 px-1.5">
                                <Cpu className="h-3 w-3 mr-1"/>{product.ram}
                            </Badge>
                        )}
                        {/* Cores só se for aparelho ou se houver espaço/necessidade */}
                        {!product.ram && colorString && (
                             <div className="flex items-center gap-1 text-[10px]">
                                <Palette className="h-3 w-3" /> <span className="line-clamp-1">{colorString}</span>
                             </div>
                        )}
                    </div>
                ) : (
                    // Se for acessório sem specs, não renderiza nada para economizar espaço
                    null
                )}
                ...
                <div className="mt-auto pt-2">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button
                            variant="outline"
                            size="icon"
                            className="h-9 w-9 shrink-0 border-primary/20 text-primary hover:bg-primary hover:text-white" 
